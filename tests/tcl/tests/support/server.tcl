...
proc start_server_error {config_file error} {
    set err {}
    append err "Cant' start the ${::SERVER_NAME}\n"
...
proc ping_server {host port} {
    puts "ping_server host=${host} port=${port}"
    set retval 0
    if {[catch {
        if {[catch {set fd [socket $host $port]} err]} {
            puts "ping_server connect host:$host port:$port error:$err"
            return $retval
        }
        ...
        puts "ping_server caught error=${e}\n"
        if {$::verbose} {
            puts -nonewline "."
        }
    } else {
...
proc server_is_up {host port retrynum} {
    puts "server_is_up host=${host} port=${port} retrynum=${retrynum}"
...
proc start_server {options {code undefined}} {
    ...
    if {![file exists $stdout]} {
        set fh [open $stdout w]
        close $fh
    }

    puts "exec file: ${::SERVER}"

    if {$::valgrind} {
        set pid [exec valgrind --suppressions=src/valgrind.sup --show-reachable=no --show-possibly-lost=no --leak-check=full ${::SERVER} $config_file > $stdout 2> $stderr &]
    } else {
        set pid [exec ${::SERVER} -c $config_file > $stdout 2> $stderr &]
    }


    puts "Starting ---- port = ${::port} pid = ${pid}"
