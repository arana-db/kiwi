...
impl RuntimeManager {
    ...
    pub async fn start(&mut self) -> Result<(), DualRuntimeError> {
        ...
        match current_state {
            LifecycleState::Running => {
                return Err(DualRuntimeError::lifecycle("RuntimeManager is already running".to_string()));
            }
            LifecycleState::Starting => {
                return Err(DualRuntimeError::lifecycle("RuntimeManager is already starting".to_string()));
            }
            LifecycleState::Stopping => {
                return Err(DualRuntimeError::lifecycle("RuntimeManager is currently stopping".to_string()));
            }
            _ => {}
        }
        ...
        *self.state.write().await = LifecycleState::Starting;
        ...
        let network_runtime = match self.create_network_runtime() {
            Ok(runtime) => {
                info!("Network runtime created with {} threads", self.config.network_threads);
                runtime
            }
            Err(e) => {
        ...
        let storage_runtime = match self.create_storage_runtime() {
            Ok(runtime) => {
                info!("Storage runtime created with {} threads", self.config.storage_threads);
                runtime
            }
            Err(e) => {
    ...
    pub async fn wait_for_state(&self, target_state: LifecycleState, timeout: Duration) -> Result<(), DualRuntimeError> {
    ...
    fn create_network_runtime(&self) -> Result<Runtime, DualRuntimeError> {
        Builder::new_multi_thread()
            ...
            .enable_all()
            .build()
            .map_err(|e| DualRuntimeError::network_runtime(format!("Failed to create network runtime: {}", e)))
    }
    ...
    fn create_storage_runtime(&self) -> Result<Runtime, DualRuntimeError> {
        Builder::new_multi_thread()
            ...
            .enable_all()
            .build()
            .map_err(|e| DualRuntimeError::storage_runtime(format!("Failed to create storage runtime: {}", e)))
    }
        ...
        match self.network_handle() {
            Ok(handle) => {
                ...
                match result {
                    ...
                    Err(e) => RuntimeHealth::Degraded(format!("Network runtime task failed: {}", e)),
                }
            }
    ...
    async fn check_storage_health(&self) -> RuntimeHealth {
        match self.storage_handle() {
            Ok(handle) => {
                ...
                match result {
                    Ok(_) => RuntimeHealth::Healthy,
                    Err(e) => RuntimeHealth::Degraded(format!("Storage runtime task failed: {}", e)),
                }
            }
        ...
        let handle = tokio::spawn(async move {
            ...
            loop {
                ...
                if !matches!(current_state, LifecycleState::Running) {
                    debug!("Health monitoring stopped due to state change: {:?}", current_state);
                    break;
                }
...
mod tests {
    ...
    async fn test_runtime_manager_wait_for_state() {
        ...
        let result = manager.wait_for_state(LifecycleState::Running, Duration::from_millis(50)).await;
