...
async fn main() {
    ...
    for result in &results {
        if result.operations_per_second < 1000.0 {
            println!("⚠️  {} shows low throughput ({:.0} ops/sec)", result.test_name, result.operations_per_second);
        }
        if result.p99_latency_ms > 100.0 {
            println!("⚠️  {} shows high P99 latency ({:.2}ms)", result.test_name, result.p99_latency_ms);
        }
        if result.success_rate < 0.99 {
            println!("⚠️  {} shows low success rate ({:.1}%)", result.test_name, result.success_rate * 100.0);
        }
    }
