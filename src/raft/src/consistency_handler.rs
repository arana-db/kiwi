...
impl ConsistencyHandler {
    ...
    async fn confirm_read_index(&self) -> RaftResult<()> {
        ...
        while start_time.elapsed() < self.config.read_index_timeout {
            ...
            if current_applied >= read_index {
                log::debug!("Read index confirmed: read_index={}, applied_index={}, elapsed={:?}", 
                           read_index, current_applied, start_time.elapsed());
                return Ok(());
            }
    ...
    async fn send_read_index_heartbeat(&self, read_index: u64) -> RaftResult<bool> {
        ...
        log::debug!("Read index heartbeat: read_index={}, total_nodes={}, reachable={}, majority_size={}, confirmed={}", 
                   read_index, total_nodes, reachable_count, majority_size, can_confirm);
    ...
    async fn check_staleness(&self) -> RaftResult<()> {
        ...
        if let Some(leader_id) = metrics.current_leader {
            ...
            if staleness <= self.config.max_staleness {
                log::debug!("Staleness check passed - staleness={:?}, max_allowed={:?}, leader={}", 
                           staleness, self.config.max_staleness, leader_id);
                Ok(())
            } else {
                log::warn!("Data too stale - staleness={:?}, max_allowed={:?}", 
    ...
    async fn calculate_staleness(&self, metrics: &RaftMetrics) -> RaftResult<Duration> {
        ...
        log::debug!("Staleness calculation: lag={}, estimated={:?}, additional={:?}, total={:?}", 
                   lag, estimated_staleness, additional_staleness, total_staleness);
    ...
    fn analyze_command_consistency(&self, command: &str) -> ConsistencyLevel {
        ...
        match self.get_command_category(&cmd) {
            ...
            CommandCategory::Unknown => {
                ...
                log::warn!("Unknown command '{}', defaulting to linearizable consistency", command);
                ConsistencyLevel::Linearizable
            }
        }
    }
    ...
    fn get_command_category(&self, command: &str) -> CommandCategory {
        match command {
            ...
            "hget" | "hmget" | "hlen" | "hexists" | "hkeys" | "hvals" | "hgetall" => CommandCategory::CriticalRead,
            
            // Reads that can tolerate some staleness
            "strlen" | "getrange" | "getbit" |
            "lpos" | "srandmember" | "sdiff" | "sinter" | "sunion" |
            "zrevrange" | "zrevrangebyscore" | "zrevrank" | "zlexcount" | "zrangebylex" => CommandCategory::CacheableRead,
            
            // Administrative commands
            "info" | "config" | "client" | "debug" | "memory" | "latency" |
            "slowlog" | "monitor" | "sync" | "psync" => CommandCategory::Administrative,
            
            // Monitoring and diagnostic commands
            "ping" | "echo" | "time" | "lastsave" | "dbsize" |
            "keys" | "scan" | "hscan" | "sscan" | "zscan" |
            "randomkey" | "dump" | "restore" => CommandCategory::Monitoring,
    ...
    pub async fn validate_consistency_request(&self, requested: ConsistencyLevel) -> RaftResult<ConsistencyValidation> {
        let cluster_health = self.assess_cluster_health().await?;
        ...
        match requested {
            ConsistencyLevel::Linearizable => {
                ...
                match cluster_health {
                    ...
                    ClusterHealthStatus::Unstable => {
                        Ok(ConsistencyValidation::Degraded {
...
mod tests {
    ...
    fn test_command_category_classification() {
        ...
        assert_eq!(handler.get_command_category("lpush"), CommandCategory::Write);
        
        // Test critical read commands
        assert_eq!(handler.get_command_category("get"), CommandCategory::CriticalRead);
        assert_eq!(handler.get_command_category("mget"), CommandCategory::CriticalRead);
        assert_eq!(handler.get_command_category("exists"), CommandCategory::CriticalRead);
        
        // Test cacheable read commands
        assert_eq!(handler.get_command_category("strlen"), CommandCategory::CacheableRead);
        assert_eq!(handler.get_command_category("srandmember"), CommandCategory::CacheableRead);
        
        // Test administrative commands
        assert_eq!(handler.get_command_category("info"), CommandCategory::Administrative);
        assert_eq!(handler.get_command_category("config"), CommandCategory::Administrative);
        
        // Test monitoring commands
        assert_eq!(handler.get_command_category("ping"), CommandCategory::Monitoring);
        assert_eq!(handler.get_command_category("scan"), CommandCategory::Monitoring);
        
        // Test unknown commands
        assert_eq!(handler.get_command_category("unknown_command"), CommandCategory::Unknown);
    }
    ...
    fn test_consistency_level_analysis() {
        ...
        assert_eq!(handler.analyze_command_consistency("set"), ConsistencyLevel::Linearizable);
        assert_eq!(handler.analyze_command_consistency("del"), ConsistencyLevel::Linearizable);
        
        // Critical reads should be linearizable
        assert_eq!(handler.analyze_command_consistency("get"), ConsistencyLevel::Linearizable);
        assert_eq!(handler.analyze_command_consistency("exists"), ConsistencyLevel::Linearizable);
        
        // Administrative commands should be eventual
        assert_eq!(handler.analyze_command_consistency("info"), ConsistencyLevel::Eventual);
        assert_eq!(handler.analyze_command_consistency("ping"), ConsistencyLevel::Eventual);
        
        // Unknown commands should default to linearizable
        assert_eq!(handler.analyze_command_consistency("unknown"), ConsistencyLevel::Linearizable);
    }
