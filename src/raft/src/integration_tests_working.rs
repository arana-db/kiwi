...
async fn test_state_machine_snapshot_operations() {
    ...
    let _responses = state_machine
        .apply_redis_commands_batch(&set_requests)
        .await
        .unwrap();
...
async fn test_compatibility_layer_integration() {
    ...
    let set_request = ClientRequest {
        ...
        command: RedisCommand::new(
            "SET".to_string(),
            vec![
                Bytes::from("integration_key"),
                Bytes::from("integration_value"),
            ],
        ),
        consistency_level: ConsistencyLevel::Linearizable,
    };
    ...
    let get_request = ClientRequest {
        id: RequestId::new(),
        command: RedisCommand::new("GET".to_string(), vec![Bytes::from("integration_key")]),
        consistency_level: ConsistencyLevel::Linearizable,
    };
...
async fn test_redis_command_types() {
    ...
    let exists_request = ClientRequest {
        id: RequestId::new(),
        command: RedisCommand::new("EXISTS".to_string(), vec![Bytes::from("nonexistent_key")]),
        consistency_level: ConsistencyLevel::Linearizable,
    };

    let response = state_machine
        .apply_redis_command(&exists_request)
        .await
        .unwrap();
    assert!(response.result.is_ok());
    if let Ok(result) = &response.result {
    ...
    let del_request = ClientRequest {
        id: RequestId::new(),
        command: RedisCommand::new("DEL".to_string(), vec![Bytes::from("nonexistent_key")]),
        consistency_level: ConsistencyLevel::Linearizable,
    };
...
async fn test_error_handling() {
    ...
    let response = state_machine
        .apply_redis_command(&invalid_set_request)
        .await
        .unwrap();
    ...
    let response = state_machine
        .apply_redis_command(&invalid_get_request)
        .await
        .unwrap();
    assert!(response.result.is_err());
    ...
    let response = state_machine
        .apply_redis_command(&unsupported_request)
        .await
        .unwrap();
    assert!(response.result.is_err());
}
