...
mod tests {
    ...
    use crate::placeholder_types::{RespData, CommandType};
    use bytes::Bytes;
    ...
    fn test_resp_data_to_redis_command_inline() {
        let resp_data = RespData::Inline(vec![
            "GET".to_string(),
            "key".to_string(),
        ]);
    ...
    fn test_command_classification() {
        ...
        let read_commands = vec![
            "GET", "MGET", "EXISTS", "TTL", "TYPE", "STRLEN",
            "LLEN", "LINDEX", "SCARD", "ZCARD", "HGET", "PING"
        ];
        
        for cmd_name in read_commands {
            let cmd = RedisCommand::new(cmd_name.to_string(), vec![Bytes::from("key")]);
            assert!(CommandSerializer::is_read_only_command(&cmd), 
                   "Command {} should be read-only", cmd_name);
            assert!(!CommandSerializer::is_write_command(&cmd),
                   "Command {} should not be write command", cmd_name);
        }
        
        // Test write commands
        let write_commands = vec![
            "SET", "DEL", "INCR", "LPUSH", "SADD", "ZADD", "HSET"
        ];
        
        for cmd_name in write_commands {
            let cmd = RedisCommand::new(cmd_name.to_string(), vec![Bytes::from("key")]);
            assert!(!CommandSerializer::is_read_only_command(&cmd),
                   "Command {} should not be read-only", cmd_name);
            assert!(CommandSerializer::is_write_command(&cmd),
                   "Command {} should be write command", cmd_name);
        }
    }
    ...
    fn test_complex_command_serialization() {
        ...
        assert_eq!(deserialized.args[0], Bytes::from("return redis.call('get', KEYS[1])"));
    }
}