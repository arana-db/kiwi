...
async fn test_log_replication_single_node() {
    ...
    assert!(
        metrics.last_applied.unwrap().index > 0,
        "At least one entry should be applied"
    );
...
async fn test_state_machine_application() {
    ...
    let metrics = node.get_metrics().await.unwrap();
    assert!(
        metrics.last_applied.unwrap().index >= 3,
        "All commands should be applied"
    );
...
async fn test_snapshot_installation() {
    ...
    // Wait for node to become leader
    node1
        .wait_for_election(Duration::from_secs(5))
        .await
        .unwrap();
    ...
    assert!(
        ...
        "Initial snapshot should have empty data"
    );
...
async fn test_log_replication_with_multiple_entries() {
    ...
    let metrics = node.get_metrics().await.unwrap();
    assert!(
        metrics.last_applied.unwrap().index >= 20,
        "All 20 entries should be applied"
    );
...
async fn test_concurrent_log_replication() {
    ...
    assert!(
        success_count >= 8,
        "Most concurrent requests should succeed (got {})",
        success_count
    );
...
async fn test_log_compaction_after_snapshot() {
    ...
    assert!(
        ...
        "System should work after log compaction"
    );
