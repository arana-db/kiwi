...
fn create_test_config(node_id: NodeId, temp_dir: &TempDir) -> ClusterConfig {
    ClusterConfig {
        enabled: true,
        node_id,
        cluster_members: vec![format!("{}:127.0.0.1:{}", node_id, 7380 + node_id)]
            .into_iter()
            .collect(),
...
async fn test_node_restart_with_state_recovery() {
    ...
    // Phase 1: Start node and write data
    {
        ...
        node.wait_for_election(Duration::from_secs(5))
            .await
            .unwrap();
        ...
        assert!(
            ...
            "Should have applied at least 5 entries"
        );
    ...
    // Phase 2: Restart node and verify state recovery
    {
        ...
        assert!(
            ...
            "Should have recovered applied state"
        );
        ...
        assert!(
            ...
            "Should be able to read after restart"
        );
        assert_eq!(
...
async fn test_log_replay_after_restart() {
    ...
    // Phase 2: Restart and verify log replay
    {
        ...
        for i in 0..10 {
            ...
            assert!(
                response.result.is_ok(),
                "Should read key {} after replay",
                i
            );
            assert_eq!(
                response.result.unwrap(),
                Bytes::from(format!("replay_value_{}", i)),
                "Should have correct value for key {}",
                i
            );
        }
...
async fn test_snapshot_recovery_correctness() {
    ...
    // Phase 2: Restart and verify snapshot recovery
    {
        ...
        assert!(
            metrics.last_applied.is_some(),
            "Should have recovered from snapshot"
        );
        ...
        for i in 0..15 {
            ...
            assert!(
                response.result.is_ok(),
                "Should read key {} after snapshot recovery",
                i
            );
            assert_eq!(
                response.result.unwrap(),
                Bytes::from(format!("snapshot_value_{}", i)),
                "Should have correct value for key {} after snapshot recovery",
                i
            );
        }
...
async fn test_recovery_with_empty_state() {
    ...
    assert!(
        response.result.is_ok(),
        "Should accept writes with empty state"
    );
...
async fn test_recovery_after_multiple_restarts() {
    ...
    for cycle in 0..3 {
        ...
        assert!(
            response.result.is_ok(),
            "Should read data from cycle {} after multiple restarts",
            cycle
        );
        assert_eq!(
            response.result.unwrap(),
            Bytes::from(format!("cycle_value_{}", cycle)),
            "Should have correct value from cycle {}",
            cycle
        );
    }
...
async fn test_incremental_recovery() {
    ...
    for i in 0..total_keys {
        ...
        assert!(
            response.result.is_ok(),
            "Should read incremental key {} after recovery",
            i
        );
        assert_eq!(
            response.result.unwrap(),
            Bytes::from(format!("incremental_value_{}", i)),
            "Should have correct incremental value for key {}",
            i
        );
    }
