...
butil::Status Raft::Init(std::string& group_id, bool initial_conf_is_null) {
  ...
  auto port = kiwi::Config::GetInstance().port + kiwi::Config::GetInstance().raft_port_offset;
  ...
  // FIXME: kiwi::Config::GetInstance().ip is default to 127.0.0.0, which may not work in cluster.
  raw_addr_ = kiwi::Config::GetInstance().ip + ":" + std::to_string(port);
  butil::ip_t ip;
  auto ret = butil::str2ip(kiwi::Config::GetInstance().ip.c_str(), &ip);
  if (ret != 0) {
    server_.reset();
  ...
  std::string prefix = "local://" + kiwi::Config::GetInstance().db_path + std::to_string(db_id_) + "/_raft";
...
std::string Raft::GetLeaderAddress() const {
  ...
  id.addr.port -= kiwi::Config::GetInstance().raft_port_offset;
...
void Raft::SendNodeAddRequest(PClient* client) {
  ...
  auto port = kiwi::Config::GetInstance().port + kiwi::Config::GetInstance().raft_port_offset;
  auto raw_addr = kiwi::Config::GetInstance().ip + ":" + std::to_string(port);
...
void Raft::CheckRocksDBConfiguration(PClient* client, PClient* join_client, const std::string& reply) {
  ...
  int current_databases_num = kiwi::Config::GetInstance().databases;
  int current_rocksdb_num = kiwi::Config::GetInstance().db_instance_num;
