...
async fn test_basic_log_store_operations() {
    ...
    let mut log_store = RocksdbLogStore::new(engine.clone())
        .expect("Failed to create log store");

    // Initially, log state should be empty
    let initial_state = log_store.get_log_state().await
        .expect("Failed to get log state");
    assert!(initial_state.last_log_id.is_none(), "Initial last_log_id should be None");
    assert!(initial_state.last_purged_log_id.is_none(), "Initial last_purged_log_id should be None");
    ...
    let state_after_append = log_store.get_log_state().await
        .expect("Failed to get log state");
    assert!(state_after_append.last_log_id.is_some(), "last_log_id should be Some after append");
    assert_eq!(state_after_append.last_log_id.unwrap().index, 5, "last_log_id should be 5");

    // Read back the entries
    let mut reader = log_store.get_log_reader().await;
    let read_entries = reader.try_get_log_entries(1..=5).await
        .expect("Failed to read entries");
    
    assert_eq!(read_entries.len(), 5, "Should read back 5 entries");
    for (i, entry) in read_entries.iter().enumerate() {
        assert_eq!(entry.log_id.index, (i + 1) as u64, "Entry index should match");
    }
}
...
async fn test_node_restart_data_recovery() {
    ...
    // Phase 1: Create log store, append entries, save vote and committed
    {
        ...
        log_store.save_committed(committed).await
            .expect("Failed to save committed");

        // Verify data is present
        let state = log_store.get_log_state().await.expect("Failed to get log state");
        assert_eq!(state.last_log_id.unwrap().index, 10, "Should have 10 entries");
        
        let read_vote = log_store.read_vote().await.expect("Failed to read vote");
        assert!(read_vote.is_some(), "Vote should be present");
        
        let read_committed = log_store.read_committed().await.expect("Failed to read committed");
    ...
    {
        let mut log_store = RocksdbLogStore::new(engine.clone())
            .expect("Failed to create log store after restart");
        ...
        assert_eq!(state.last_log_id.unwrap().index, 10, "Should still have 10 entries after restart");

        // Verify entries can be read
        let mut reader = log_store.get_log_reader().await;
        let entries = reader.try_get_log_entries(1..=10).await
            .expect("Failed to read entries after restart");
        assert_eq!(entries.len(), 10, "Should read back all 10 entries after restart");
        ...
        assert!(committed.is_some(), "Committed should persist across restart");
...
async fn test_node_restart_after_truncate() {
    ...
    // Phase 1: Append entries and truncate
    {
        ...
        log_store.truncate(truncate_log_id).await
    ...
    // Phase 2: Restart and verify truncation persisted
    {
        ...
        assert_eq!(entries.last().unwrap().log_id.index, 5, "Last entry should be index 5");
    }
...
async fn test_node_restart_after_purge() {
    ...
    // Phase 1: Append entries and purge
    {
        ...
        log_store.purge(purge_log_id).await
            .expect("Failed to purge");

        // Verify purge worked
        let state = log_store.get_log_state().await
            .expect("Failed to get log state");
        assert_eq!(state.last_purged_log_id.unwrap().index, 4, "last_purged should be 4");
    ...
    // Phase 2: Restart and verify purge persisted
    {
        ...
        let state = log_store.get_log_state().await
            .expect("Failed to get log state after restart");
        assert!(state.last_purged_log_id.is_some(), "last_purged should persist");
        assert_eq!(state.last_purged_log_id.unwrap().index, 4, "last_purged should still be 4");
        ...
        assert_eq!(entries.first().unwrap().log_id.index, 5, "First entry should be index 5");
    }
...
async fn test_append_after_restart() {
    ...
    // Phase 2: Restart and append more entries
    {
        ...
        let mut reader = log_store.get_log_reader().await;
        let all_entries = reader.try_get_log_entries(1..=10).await
            .expect("Failed to read entries");
        assert_eq!(all_entries.len(), 10, "Should have 10 entries total");
        
        // Verify continuity
        for (i, entry) in all_entries.iter().enumerate() {
            assert_eq!(entry.log_id.index, (i + 1) as u64, "Entries should be continuous");
        }
    }
...
async fn test_multiple_restarts_with_operations() {
    ...
    // Cycle 2: Restart, truncate, and append
    {
        ...
        // Truncate at 6
        log_store.truncate(LogId::new(LeaderId::new(1, 1), 6)).await
            .expect("Failed to truncate");
    ...
    // Cycle 3: Restart and verify
    {
        ...
        assert_eq!(entries[4].log_id.leader_id.term, 1, "Entry 5 should have term 1");
        assert_eq!(entries[5].log_id.leader_id.term, 2, "Entry 6 should have term 2");
    }
    ...
    // Cycle 4: Restart, purge, and verify
    {
        ...
        log_store.purge(LogId::new(LeaderId::new(1, 1), 5)).await
            .expect("Failed to purge");
    ...
    // Cycle 5: Final restart and verify
    {
        ...
        let state = log_store.get_log_state().await
            .expect("Failed to get log state");
        assert_eq!(state.last_purged_log_id.unwrap().index, 5, "Purge should persist");
        assert_eq!(state.last_log_id.unwrap().index, 10, "Last log should be 10");
...
async fn test_empty_database_restart() {
    ...
    // Phase 2: Restart and verify still empty
    {
        ...
        let state = log_store.get_log_state().await
            .expect("Failed to get log state after restart");
        assert!(state.last_log_id.is_none(), "Should still be empty after restart");
        assert!(state.last_purged_log_id.is_none(), "Should still have no purge after restart");
...
async fn test_large_entry_persistence() {
    ...
    let large_binlog_entries: Vec<BinlogEntry> = (0..100)
        .map(|i| BinlogEntry {
            cf_idx: i % 6,
            op_type: if i % 2 == 0 { OperateType::Put } else { OperateType::Delete },
    ...
    // Phase 2: Restart and verify large entry
    {
        ...
        let mut reader = log_store.get_log_reader().await;
        let entries = reader.try_get_log_entries(1..=1).await
