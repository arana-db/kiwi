...
设计启动时还存在两个需要纳入 #349 交付链路的生命周期问题：
...
采用方案 A，并通过两个独立 PR 完成交付：

1. 第一阶段 PR #358 先修复 callback/factory 对 DB 的强引用环，并以真实同路径 reopen 测试验证 RocksDB 可以释放文件句柄。该 PR 是 #349 的前置生命周期修复，只引用 #349，不关闭 Issue。
2. 第二阶段后续 PR 完成具体类型迁移、删除错误的 wrapper Drop、删除 `Engine` trait 和 `engine` crate，并完成完整回归验证。只有这一阶段达到本文成功标准后才关闭 #349。

不采用只做机械类型替换的方案，因为它会保留 DB 强引用环和不准确的 close 语义，无法满足 #349 的 shutdown owner 验收条件。

两个 PR 共享同一最终所有权模型，但保持独立审查和验证边界：#358 只改变 callback/factory 的引用方向，不提前改动 `Engine` 相关类型；后续 PR 在已验证的无环生命周期模型上完成方案 A 的其余迁移。分期只改变交付顺序，不改变方案 A 的最终技术目标和成功标准。

## 执行分段

实施分为两个独立 PR：先完成强引用环修复，再继续删除 `Engine` trait：

### 第一阶段 PR #358：打破 DB 强引用环
...
- 不在第一阶段删除 `need_close` 或修改 `RocksdbEngine::Drop`；这些属于第二阶段具体所有权迁移。

第一阶段必须通过 storage 定向测试、storage crate 测试、格式检查、跨平台 CI 和代码审查。PR #358 合并且证据充分后才进入第二阶段。

### 第二阶段后续 PR：完成方案 A 并关闭 #349

继续执行本文其余设计：迁移到具体 `Arc<DB>` / `&DB`、删除错误的 wrapper Drop、删除 `Engine` trait 和 crate，并完成 snapshot hot-swap 与全 workspace 门禁。

第二阶段达到本文成功标准后关闭 #349。两个 PR 的分期只改变实施顺序，不改变 #349 的最终目标和成功标准。

## 具体类型模型

### Redis 主数据 DB

`Redis` 直接持有一个不可 Clone 的具体 RocksDB shutdown owner。该 owner 内部保存
`Arc<DB>`，只向调用方提供受 `Redis` 生命周期约束的 `&DB`：
...
`Redis::open()` 直接构造唯一 owner，并由 owner 内部创建 `Arc<DB>`：

```rust
let db = RocksDbOwner::new(DB::open_cf_descriptors(
    &db_opts,
    db_path,
    column_families,
)?);
...
`RocksDbOwner` 可以实现 `Deref<Target = DB>` 以保留原生 RocksDB 方法调用，但不得
实现 `Clone`、不得暴露内部 `Arc<DB>`，也不得增加一组同名转发函数来模拟已删除的
trait。对外如需检查或操作 DB，只返回 `Option<&DB>`；长期 owner 必须保存
`Arc<Redis>`，不能绕过 Redis 保存 `Arc<DB>`。
...
单个正在执行的 compaction filter 可以在其有限生命周期内持有升级得到的 `Arc<DB>`；
filter 结束后必须释放该引用。factory 和 listener 不得永久持有强引用。

有限生命周期不等于可以忽略最终析构线程：如果 Redis owner 已经释放，filter 的临时
`Arc<DB>` 可能成为最后 owner，并在 compaction worker 上执行 `rocksdb_close()`；底层
close 又会等待当前 compaction 计数归零，形成自等待。因此 Redis 的唯一 shutdown
owner 在释放自身强引用前必须先取消并等待后台任务，使 filter 的临时引用在 owner 仍
存活时完成并释放。

## Shutdown 与所有权责任

### 删除可克隆 owner 的隐式 cancel
...
普通共享引用释放不应改变其他 owner 可观察到的 DB 行为。Raft LogStore 等可 Clone
类型不得实现主动 shutdown。

Storage 主数据 DB 是例外：不可 Clone 的 `RocksDbOwner` 是唯一 shutdown owner，其
`Drop` 在仍持有内部 `Arc<DB>` 时调用：

```rust
self.db.cancel_all_background_work(true);
```

该调用用于等待 event listener、flush 和 compaction callback 退出，避免最后一个 DB
引用在 callback worker 上析构；它不能迁移到 `Arc<DB>` clone、`Arc<Redis>` clone 或
其他共享 wrapper 上。

### Redis 与 Storage

删除旧 `Redis.need_close`、`set_need_close()` 和基于该标志的 `Redis::drop()` 清理逻辑。
`RocksDbOwner::drop()` 负责唯一 shutdown；`Arc<Redis>` 引用计数决定何时真正进入该
shutdown。只要仍有外部 `Arc<Redis>`，Storage 释放自身 owner 不得提前取消 DB。
...
`RocksDbOwner::drop()` 先持有唯一长期 `Arc<DB>` 调用
`cancel_all_background_work(true)`，等待 callback 的临时强引用退出，再释放内部
`Arc<DB>`。随后由 rust-rocksdb 的 `DB::drop` 完成真实关闭和文件句柄释放。Raft
LogStore 没有 callback 反向强引用路径，仍依赖最后一个普通 `Arc<DB>` 的自然析构，
不得增加主动 cancel。
...
6. 阻塞一个真实 compaction filter 时，释放最后一个 `Arc<Redis>` 必须等待 filter；
   释放 filter 后 owner drop 在限定时间内完成，并可从同一路径 reopen。
...
- Storage 主数据 DB 的唯一 shutdown owner 会在释放最后长期引用前等待 callback 完成，
  不会把最终 DB 析构留给 compaction worker。
