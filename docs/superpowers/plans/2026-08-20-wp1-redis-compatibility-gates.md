...
在 `manifest.rs` 定义唯一共享的 `canonical_core_required_jobs_bytes`，由 Core helper
和 `oracle.rs` offline replay 直接复用；controller 另有逐字节兼容的 Python 实现，并由
同一 golden fixture 锁定。`canonical-v1` 字节算法固定为：

- 输入只允许 strict JSON object、array、string、boolean 和 `0..=u64::MAX` integer；
  禁止 null、float、non-finite number 和 duplicate key；
- object key 必须是 ASCII，并在每一层按无符号 UTF-8 byte lexicographic order 递归排序；
  array 保留 registry validator 已冻结的顺序；
- 输出是无 BOM 的 strict UTF-8；object/array 使用 compact comma/colon separators，
  不包含其他 whitespace；不转义 `/`，非 ASCII string scalar 直接输出 UTF-8；
- quote、reverse solidus、U+0008、U+0009、U+000A、U+000C、U+000D 使用 JSON standard
  short escape；其他 U+0000..U+001F 使用 `u00` 加两个小写十六进制数字的 JSON Unicode
  escape，禁止 surrogate scalar；
- integer 使用无符号十进制最短形式，不允许正号或前导零；文档末尾恰好一个 LF。

Rust/Python golden test 必须至少包含嵌套乱序 object、非 ASCII UTF-8、quote/reverse
solidus、全部 short escapes、其他 C0 control 和 `u64::MAX`，并断言 Rust helper bytes、
Python profile-specific canonicalizer bytes、固定 Base64、size 和 SHA-256 完全相等。
Key order、任一 whitespace、CRLF/缺 LF/多 LF、Unicode escape 代替非 ASCII UTF-8、
escape hex 大小写或整数格式漂移都必须失败。现有 Vector helper/controller
canonicalization 不修改。
...
core_registry_canonical_bytes_match_rust_python_golden_fixture
core_replay_requires_oracle_final_output_variables
...
file/schema/size/SHA、`canonical_bytes_base64`，以及 strict-parsed required-jobs document。
Controller 先保留 helper stdout 原始 bytes，strict-parse 后使用 Core profile-specific
canonicalizer 重新生成 bytes，并要求与原始文件逐字节相等，之后才记录 binding。Offline
replay strict-decode `canonical_bytes_base64`，调用与 helper 共享的
`canonical_core_required_jobs_bytes(document)`，先要求 regenerated bytes 与 decoded bytes
逐字节相等，再重算 size/SHA，并与 `registry_binding`、callback manifest 和 helper identity
全部相等；这样进程和 `/work` 删除后仍能证明最终 evidence 消费的是受冻结
source/helper pairing，而不是等价但字节不同的 JSON 或调用者注入的预生成 JSON。
...
`DifferentialEvidenceIdentity` 接受上述两个精确 schema 与文件名 pairing，禁止 wildcard。Vector fixture serialized fields和验证结果保持不变。新增 ignored test `replay_published_core_evidence_after_cleanup` 统一从现有 wrapper contract `KIWI_REDIS_ORACLE_OUTPUT`、`KIWI_REDIS_ORACLE_EVIDENCE_OUTPUT`、`KIWI_EXPECTED_HEAD`、`KIWI_EXPECTED_TREE` 读取最终文件，并调用同一 `verify_external_bindings` 路径；不得新增 `KIWI_CORE_FINAL_*` 别名。Test 必须拒绝任一变量缺失、非 absolute path、两个 output 指向同一路径、路径与 provenance evidence identity 不一致，或文件不是 regular final file。如果实现需要修改 provenance v4 field set 或 sealed external binding，停止实施并回到 planning；Issue #433 不授权 silent v5。
...
- helper artifact 严格符合 `canonical-v1` byte algorithm，controller 保留原始 bytes 并与 profile-specific regenerated bytes 逐字节比较；
- collection、raw transcript、final state、summary 和 evidence collector 全部只消费该 canonical file；
- offline semantic replay strict-decode `canonical_bytes_base64`，用共享 Rust routine 逐字节比较后再验证 size/SHA、source manifest entry 和 helper identity；
- final outputs 只使用 `KIWI_REDIS_ORACLE_OUTPUT` 和 `KIWI_REDIS_ORACLE_EVIDENCE_OUTPUT`，并拒绝缺失、相同或与 provenance identity 不一致的 paths；
...
canonical key order/whitespace/newline/UTF-8/escape/integer drift、canonical bytes Base64/size/
SHA/document drift、registry copy drift、final output variable missing/same-path/provenance-path
mismatch、summary 字段删除、30→29、skip/xfail/xpass/deselect、validation reordering、
cleanup bypass、unexpected file、symlink、missing artifact、expected Head mismatch、required job
注入 test-mode env、test-mutant mode 意外发布 artifact。
...
预期：runner source、canonical registry producer/byte algorithm、final-output variable
contract、collection/summary、test-mode 隔离、cleanup、artifact allowlist 和 expected-Head
mutants 全部通过。Task 1 总体 gap test 仍因 workflow 尚未创建保持 RED，不运行完整
`ci_contract` suite。
...
test "$KIWI_REDIS_ORACLE_OUTPUT" != "$KIWI_REDIS_ORACLE_EVIDENCE_OUTPUT"
cargo test -p kiwi-compat --test oracle \
...
- canonical registry 改 key order、增加 whitespace、改 LF/CRLF、把非 ASCII UTF-8 改成 Unicode escape 或改变 C0 escape，再重算外层 binding；
- canonical bytes Base64 与 parsed document 不一致，再重算外层 binding；
...
- 删除 `KIWI_REDIS_ORACLE_OUTPUT` 或 `KIWI_REDIS_ORACLE_EVIDENCE_OUTPUT`，令两者同 path，或令路径与 provenance evidence identity 不一致；
