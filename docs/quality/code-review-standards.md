...
| **建议** | 🟡 `suggestion` | 应修复。包括低风险的命名/逻辑不清、局部性能问题（如不必要克隆）、应抽取的重复代码和非关键测试补充。 | 尽量在本次 PR 修复；否则开跟踪 issue |
| **建议讨论** | 🟠 `question` | 意图不明确，需作者澄清。不要在未理解意图时假定代码错误。 | 作者回复后决定是否改动 |
| **细节** | 💭 `nit` | 风格、命名微调、文档补充、可选的替代写法。 | 由作者自行决定，不阻塞 |

严重程度必须按实际影响判定，不能只按问题类别机械归档。外部输入校验缺失、无界资源增长或关键行为缺少回归测试，若可能造成安全、正确性或稳定性问题，均应标为 `blocker`。
...
- 🔴 生产路径禁止 `unwrap()` / `panic!()` / `unreachable!()` / `unimplemented!()`。项目已在 `clippy` 中用 `-D clippy::unwrap_used` 禁止 `unwrap()`；其余宏由人工审查。
...
- 🔴 所有外部输入（客户端 RESP、配置文件、gRPC）必须按协议和命令语义校验，覆盖长度、整数范围和资源上限。仅对语义要求文本的字段校验 UTF-8；Redis key、value 及其他二进制字段必须保留原始字节，并按对应命令约束校验。
