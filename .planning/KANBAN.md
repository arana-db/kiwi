...
> 更新日期：2026-07-28
>
> 当前里程碑：文档基线由 PR #371 发布；M1 Cache OFF 已启动
>
> 当前运行模式：Cache OFF
>
...
| ID | 工作项 | Requirement | 当前状态 |
|---|---|---|---|
| `M1-001` | Redis 8.8.1 exact Oracle、机器可读 manifest 与 provenance | `REQ-COMPAT-001`、`003`、`004`；Task 2 另含 `REQ-LICENSE-003`、`005` | 已获实现授权。截至 2026-07-28，Task 1 已提交至 Draft PR #372，Head `2507a0c7b47707ef0f29ad360f51676976ffb483`，尚未合并；恢复时必须实时复检。Task 2 已启动但未提交、未验收。精确本机 dirty ownership 与验证状态以对应 `.codex/recovery/` 为准。 |

## Ready after M1-001 acceptance

这些卡尚未进入当前实现范围。只有 `M1-001` 完成要求的真实构建、验证、复审和验收后，才能按依赖关系另行授权并建立新的隔离工作边界；不得因为 PR #372 存在或局部测试绿色而隐式启动。
...
| `M7-002` | Embedded Redis Hot Tier 动态库 C ABI spike | `REQ-HOT-010`、`012`；`REQ-LICENSE-003`、`005`、`007` | System Stability Gate + 用户新批准 | `.so`、`.dylib`、`.dll`、import/static library 均禁止 |
...
- M0 文档以 PR #371 为发布记录；恢复时必须实时查询其状态，并确认 `main` 是否已包含对应文档基线。该发布必须保持显式文件边界，禁止带入无关 dirty 文件。
- `M1-001` 是当前唯一已授权的实现卡。截至 2026-07-28，PR #372 只包含 Task 1；恢复时必须实时复检。Task 2 的本机改动和验证状态不得从 PR 状态推断，也不得在未验收时移入 Done。
- 发布 M0 文档或推进 `M1-001` 不授权修改 Kiwi 生产源码、生产依赖、通用构建路径或 CI，也不自动授权任何后续实现卡。
