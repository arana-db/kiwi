...
- `protoc` (protobuf compiler) — required for OpenRaft. Install via `brew install protobuf` (macOS) or `apt install protobuf-compiler` (Linux).
...
# Run
cargo run --bin server         # Run server (debug)
...
- **`clippy::unwrap_used` is denied project-wide.** Use `expect()` with a descriptive message, or propagate errors with `?`/`Result`. Never use `.unwrap()`.
  - When `.unwrap()` is truly unavoidable (e.g., in tests or infallible cases), add `#[allow(clippy::unwrap_used)]` on the function or block.
